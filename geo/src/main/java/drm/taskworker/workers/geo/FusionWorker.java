...
public class FusionWorker extends AbstractGeoWorker {
	...
	public TaskResult work(Task task) {
		...
		try {
			...
			SortedMap<Region, BufferedImage> data = coorder(reg,
					imgs);

			Region[] regs = data.keySet().toArray(new Region[4]);
			ensureFit(regs);

			BufferedImage[] parts = data.values().toArray(
					new BufferedImage[4]);
	...
	private List<Region> getRegions(Task task) throws ParameterFoundException {
		...
		List<Region> regs = new LinkedList<>();
		for (ValueRef valueRef : refs) {
			regs.add((Region) valueRef.getValue());
		}
		return regs;
	}
	
	private List<BufferedImage> getImages(Task task) throws ParameterFoundException {
		List<ValueRef> refs = (List<ValueRef>) task.getParam("img");
		List<BufferedImage> regs = new LinkedList<>();
		for (ValueRef valueRef : refs) {
			regs.add(((SerialisableImageContainer) valueRef.getValue()).getImage());
		}
		return regs;
	}
	...
	private <T> SortedMap<Region, T> coorder(
			List<Region> reg, List<T> imgs) {
		SortedMap<Region, T> sorter = new TreeMap<>(
				new RegionSort());
